...
# part 2b
# Let us use Lasso Regression so that we can penalize some variables.
#  10-Fold Cross-validation

# perform k-fold cross-validation to find optimal lambda value
x <- data.matrix(train_og[,c('X15', 'X18','X19','X37','X40','X52', 'X54','X60','X65','X66','X68','X70','X82',
                             'X85','X86','X89','X90','X91')])
y <- train_og$bankrupt
...
# find coefficients fo best model
lasso.reg <- glmnet(x, y, alpha = 1, lambda = best_lambda)
coef(lasso.reg)

# we can see that lasso dropped 6 variables all together

# prediction using our train set for error
yhat.train.lasso <- predict(lasso.reg, x, 
                         type = "response") 

# classifying the deposit variable as 0/1 or no/yes with a threshold of 0.5
lasso_yhat.train.class <- ifelse(yhat.train.lasso > 0.5, 1, 0)

# confusion matrix of our train set
tab.lasso_train <- table(train_og$bankrupt, 
                         lasso_yhat.train.class, 
                      dnn = c("Actual","Predicted"))
tab.lasso_train

# overall train error 
lasso_train <- mean(lasso_yhat.train.class!= train_og$bankrupt)
lasso_train
# class 1 test error
lasso_train_class1 <- tab.lasso_train[2,1]/5269
lasso_train_class1
# class 0 test error
lasso_train_class0 <- tab.lasso_train[1,2]/5269
lasso_train_class0



# prediction using our test set for error
x2 <- data.matrix(dat.test[,c('X15', 'X18','X19','X37','X40','X52', 'X54','X60','X65','X66','X68','X70','X82',
                              'X85','X86','X89','X90','X91')])
yhat.test.lasso <- predict(lasso.reg, x2, 
                            type = "response") 

# classifying the deposit variable as 0/1 or no/yes with a threshold of 0.5
lasso_yhat.test.class <- ifelse(yhat.test.lasso > 0.5, 1, 0)

# confusion matrix of our train set
tab.lasso_test <- table(dat.test$bankrupt, 
                         lasso_yhat.test.class, 
                         dnn = c("Actual","Predicted"))
tab.lasso_test

# overall train error 
lasso_test <- mean(lasso_yhat.test.class!= dat.test$bankrupt)
lasso_test
# class 1 test error
lasso_test_class1 <- 0
# class 0 test error
...
lr_train_adas
# class 1 test error
lr_train_adas_class1 <- tab.lr_train_adas[2,1]/3500
lr_train_adas_class1
# class 0 test error
lr_train_adas_class0 <- tab.lr_train_adas[1,2]/3500
...
train.error <- data.frame(matrix(0,3,11))
names(train.error) <- c("Error" , "Logistic Reg Train", "Lasso Reg Train" ,"Logistic Reg Boot", "Logistic Reg ADAS",
                        "Decision Tree", "Pruned Tree", "Decision Tree Boot","Pruned Tree Boot",
                        "Decision Tree Adas", "Pruned Tree ADAS")

train.error[1,] <- c('Overall Error', lr_train,lasso_train, lr_train_boot, lr_train_adas, tree_og_err, pt1_err,
                     tree_boot_err, pt2_err, tree_adas_err, pt3_err)
train.error[2,] <- c('Class 1 Err', lr_train_class1,lasso_train_class1,lr_train_boot_class1, lr_train_adas_class1, 
                     tree_og_class1, pt1_class1, tree_boot_class1, pt2_class1, tree_adas_class1,
                     pt3_class1)
train.error[3,] <- c('Class 0 Err', lr_train_class0, lasso_train_class0, lr_train_boot_class0, lr_train_adas_class0, 
...
test.error <- data.frame(matrix(0,3,11))
names(test.error) <- c("Error" , "Logistic Reg","Lasso Regression","Logistic Reg Boot", "Logistic Reg ADAS",
                       "Decision Tree", "Pruned Tree", "Decision Tree Boot","Pruned Tree Boot",
                       "Decision Tree Adas", "Pruned Tree Boot")

test.error[1,] <- c('Overall Error', lr_test, lasso_test, lr_test_boot, lr_test_adas, tree_og_err.test, pt1_err.test,
                    tree_boot_err.test, pt2_err.test, tree_adas_err.test, pt3_err.test)
test.error[2,] <- c('Class 1 Err', lr_test_class1, lasso_test_class1, lr_test_boot_class1, lr_test_adas_class1, 
                    tree_og_class1.test, pt1_class1.test, tree_boot_class1.test, pt2_class1.test
                    , tree_adas_class1.test,
                    pt3_class1.test)
test.error[3,] <- c('Class 0 Err', lr_test_class0, lasso_test_class0, lr_test_boot_class0, lr_test_adas_class0, 
